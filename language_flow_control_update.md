...
int age = 18;
  ...
  /*
  In this example above, we declare an age variable with a value of 18. The if statement checks whether the age is greater than or equal to 18. 
...
**If/If-else statement in various languages:**

swift, golang and kotlin, share some similarities in their "if/if-else" statements.
...
If the condition is false, the code inside the else if block is checked, and if that condition is true, "The number is negative."
is printed. If both conditions are false, the code inside the else block is executed, which prints "The number is zero."
*/
...
**If/If-else statement in python**

In Python, parentheses around the condition in an if statement are optional. This flexibility allows for cleaner and more concise code. 
For example, instead of if (condition):, you can write if condition:
It also uses the "elif" keyword for adding additional conditions in an if-elif-else statement. This keyword combines "else" and "if" into a single keyword, making the code more readable and expressive.
...
"""
In this example, we have a variable age assigned a value of 25. The if statement checks if the age is greater than or equal to 18. If the condition evaluates to true, the code block indented under the if statement is executed, which prints the message "You are eligible to vote."

If the condition is false, the code block is skipped, and no output is generated.

You can run this code and it will output: You are eligible to vote.
...
"""
In this example, the condition x > 10 is evaluated. Since x is 5, which is not greater than 10, the condition is false. Therefore, the code block inside the else statement print("x is not greater than 10") is executed.
...
"""
In this example, the value of x is 5. The if-elif-else statement checks multiple conditions in order. If the first condition x > 10 is true, the corresponding block of code is executed. If the first condition is false, it moves to the next condition. In this case, x > 5 is true, so the second block of code is executed. If both conditions are false, the else block is executed.

Since x is 5, the condition x > 10 is false. However, the condition x > 5 is true. Therefore, the code block inside the elif statement 
print("x is greater than 5 but not greater than 10") will be executed, and the output will be: x is greater than 5 but not greater than 10
